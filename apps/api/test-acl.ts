...
async function run() {
  ...
  const aclWhereUserA: any =
    accessControlService.generateAclWhereClause(userFromCampusA);
  console.log(
    'ACL Where Clause for User A (Campus A):',
    JSON.stringify(aclWhereUserA, null, 2),
  );
  ...
  const checksCampus = hasAccessArray?.find(
    (cond: any) => cond.campusId === 'campus-a',
  );

  if (checksCampus) {
    console.log(
      'SUCCESS: The generated where clause explicitly checks for Campus A access records.',
    );
  } else {
